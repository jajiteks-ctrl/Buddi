...
from rest_framework import serializers
from django.contrib.auth.models import User
from app.models import Profile, Product, Category, Cart, CartItem, Order, OrderItem, Wishlist, WishlistItem
...
class CartItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CartItem
        fields = ["id", "product_id", "product_name", "price", "quantity", "image"]
...
class DecreaseCartItemSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        item = CartItem.objects.filter(cart=cart, product_id=validated_data["product_id"]).first()
